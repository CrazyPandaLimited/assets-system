...
namespace CrazyPanda.UnityCore.ResourcesSystem
{
	class FileLocksManager
	{
		...
		public void AddReadLock( string key )
		{
			...
			{
				throw new FileLocksManagerException( string.Format( "Cannot add lock {0} while there lock {1} exist",
																	LockReason.Read, LockReason.Write ) );
			}
		...
		public void RemoveReadLock( string key )
		{
			...
			if( !InternalHasLock( lockReason, LockReason.Read ) )
			{
				throw new FileLocksManagerException( string.Format( "Cannot remove lock {0} because this lock was not set",
																	LockReason.Read ) );
			}
		...
		public void AddWriteLock( string key )
		{
			...
			{
				throw new FileLocksManagerException( string.Format( "Cannot add lock {0} while there lock {1} exist",
																	LockReason.Write, LockReason.Read ) );
			}
			
			if( InternalHasLock( lockReason, LockReason.ManualLock ) )
			{
				throw new FileLocksManagerException( string.Format( "Cannot add lock {0} while there lock {1} exist",
																	LockReason.Write, LockReason.ManualLock ) );
			}
