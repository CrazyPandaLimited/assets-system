...
{
    public class VersionedDIskCachedManifestBasedLoaderChoiceResolver : ILoaderChoiceResolver
    {
        ...
        public IResourceLoader Resolve(string uri, out string resolvedUri)
        {
            ...
            {
                if (resourceLoader is VersionedDiskCachedResourceLoader &&
                    ((VersionedDiskCachedResourceLoader) resourceLoader).Manifest.HasResource(uri))
                {
