    ...
    public class AssetGraphEditorWindow : EditorWindow
    {
		...
		*/
        ...
        private void Init ()
        {
            ...
            var windowIcon = (EditorGUIUtility.isProSkin) ? _assetGraphGuiConfig.d_AssetGraphWindow : _assetGraphGuiConfig.AssetGraphWindow;
            ...
            this.m_showVerboseLog = UserPreference.Preferences._defaultVerboseLog;
        ...
        private void SetGraphAssetPath (string newPath)
        {
            if (newPath == null) {
                ...
                if (m_graphAssetName.Length > _assetGraphGuiConfig.TOOLBAR_GRAPHNAMEMENU_CHAR_LENGTH) {
                    m_graphAssetName = m_graphAssetName.Substring (0, _assetGraphGuiConfig.TOOLBAR_GRAPHNAMEMENU_CHAR_LENGTH) + "...";
                }
        ...
        private void DrawGUIToolBar ()
        {
            using (new EditorGUILayout.HorizontalScope (EditorStyles.toolbar)) {
                ...
                if (GUILayout.Button (new GUIContent (m_graphAssetName, "Select graph"), EditorStyles.toolbarPopup, GUILayout.Width (_assetGraphGuiConfig.TOOLBAR_GRAPHNAMEMENU_WIDTH), GUILayout.Height (_assetGraphGuiConfig.TOOLBAR_HEIGHT))) {
                ...
                if (GUILayout.Button (new GUIContent ("Refresh", m_refreshIcon, "Refresh graph status"), EditorStyles.toolbarButton, GUILayout.Width (80), GUILayout.Height (_assetGraphGuiConfig.TOOLBAR_HEIGHT))) {
                    Setup ();
                }
                GUILayout.Space (4);

                m_showErrors = GUILayout.Toggle (m_showErrors, new GUIContent (m_miniErrorIcon, "Show errors"), EditorStyles.toolbarButton, GUILayout.Height (_assetGraphGuiConfig.TOOLBAR_HEIGHT));
                m_showDescription = GUILayout.Toggle (m_showDescription, new GUIContent (m_miniInfoIcon, "Show graph description"), EditorStyles.toolbarButton, GUILayout.Height (_assetGraphGuiConfig.TOOLBAR_HEIGHT));
                m_showVerboseLog = GUILayout.Toggle (m_showVerboseLog, new GUIContent ("Verbose Log", "Increse console log messages"), EditorStyles.toolbarButton, GUILayout.Height (_assetGraphGuiConfig.TOOLBAR_HEIGHT));
                LogUtility.ShowVerboseLog (m_showVerboseLog);

                GUILayout.Space (4);

                m_controller.TargetGraph.UseAsAssetPostprocessor = GUILayout.Toggle (m_controller.TargetGraph.UseAsAssetPostprocessor, new GUIContent ("Use As Postprocessor", "Graph will be used as asset postprocessor if enabled"), EditorStyles.toolbarButton, GUILayout.Height (_assetGraphGuiConfig.TOOLBAR_HEIGHT));
                ...
                GUILayout.Label ("Platform:", tbLabel, GUILayout.Height (_assetGraphGuiConfig.TOOLBAR_HEIGHT));
                ...
                int newIndex = EditorGUILayout.Popup (currentIndex, NodeGUIUtility.supportedBuildTargetNames, 
                                   EditorStyles.toolbarPopup, GUILayout.Width (150), GUILayout.Height (_assetGraphGuiConfig.TOOLBAR_HEIGHT));
                ...
                using (new EditorGUI.DisabledScope (m_controller.IsAnyIssueFound)) {
                    if (GUILayout.Button ("Build", EditorStyles.toolbarButton, GUILayout.Height (_assetGraphGuiConfig.TOOLBAR_HEIGHT))) {
		...
		*/
        ...
        {
            float gridSize = UserPreference.Preferences._editorWindowGridSize;
