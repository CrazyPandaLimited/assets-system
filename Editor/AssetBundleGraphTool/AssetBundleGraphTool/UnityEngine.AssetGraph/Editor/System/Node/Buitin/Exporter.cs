                    ...
                    foreach (var source in inputSources)
                    {
                        ...
                        if (m_flattenDir[target] == 0)
                        {
                            ...
                            if (destinationSourcePath.StartsWith(Model.Settings.Path.BundleBuilderCachePath))
                            {
                                ...
                                var reducedArray = new string[splitted.Length - (cacheFolderDepth +1)];

                                Array.Copy(splitted, (cacheFolderDepth+1), reducedArray, 0, reducedArray.Length);
                            ...
                            if (destinationSourcePath.StartsWith(Model.Settings.Path.GraphWorkResultCachePath))
                            {
                                destinationSourcePath = destinationSourcePath.Replace(Model.Settings.Path.GraphWorkResultCachePath + Model.Settings.UNITY_FOLDER_SEPARATOR.ToString(),"");
                            }
