...
{
    [CustomNode("Group Assets/Group By File Folder", 40)]
    public class GroupingByFolder : Node, Model.NodeDataImporter
    {
        ...
        public override void OnInspectorGUI(NodeGUI node, AssetReferenceStreamManager streamManager, NodeGUIEditor editor, Action onValueChanged)
        {
            ...
            using (new EditorGUILayout.VerticalScope(GUI.skin.box))
            {
                ...
                using (disabledScope)
                {
                    ...
                    var newGroupingKeyword = string.Empty;
                    
                    if ((GroupingPatternType) m_patternType[editor.CurrentEditingGroup] == GroupingPatternType.WildCard ||
                        (GroupingPatternType) m_patternType[editor.CurrentEditingGroup] == GroupingPatternType.RegularExpression)
                    {
                        newGroupingKeyword = EditorGUILayout.TextField("Grouping Keyword", m_groupingKeyword[editor.CurrentEditingGroup]);
                    }
                    else if ((GroupingPatternType) m_patternType[editor.CurrentEditingGroup] == GroupingPatternType.RootFolder)
                    {
                        newGroupingKeyword = editor.DrawFolderSelector("Root folder", "Select Folder",
                    ...
                    switch ((GroupingPatternType) m_patternType[editor.CurrentEditingGroup])
                    {
                        ...
                        case GroupingPatternType.RegularExpression:
                            ...
                            break;
                        case GroupingPatternType.RootFolder:
                            helpText = "Grouping Keyword requires root folder";
                            break;
                    }
            ...
            ValidateGroupingKeyword(
                ...
                () =>
                {
                    throw new NodeException(
                        String.Format("Grouping Keyword must contain {0} for numbering: currently {1}", Model.Settings.KEYWORD_WILDCARD, m_groupingKeyword[target]),
                ...
                foreach (var ag in incoming)
                {
                    foreach (var g in ag.assetGroups.Keys)
                    {
                        ...
                        foreach (var a in assets)
                        {
                            ...
                            if ((GroupingPatternType) m_patternType[target] == GroupingPatternType.WildCard ||
                                (GroupingPatternType) m_patternType[target] == GroupingPatternType.RegularExpression)
                            {
                                var match = regex.Match(targetPath);
                                isSuccess = match.Success;
                            }
                            else if ((GroupingPatternType) m_patternType[target] == GroupingPatternType.RootFolder)
                            {
                                isSuccess = targetPath.StartsWith(m_groupingKeyword[target]);
                            }

                            if (isSuccess)
                            {
                                var newGroupingKey = Path.GetDirectoryName(targetPath); // match.Groups[1].Value;

                                if ((GroupingPatternType) m_patternType[target] == GroupingPatternType.RootFolder)
                                {
                                    newGroupingKey = newGroupingKey.Replace(m_groupingKeyword[target], "");
                                }

                                String[] pathParts = newGroupingKey.Split(Path.DirectorySeparatorChar);
                                    ...
                                    (GroupingPatternType) m_patternType[target] == GroupingPatternType.RegularExpression)
                                {
                                    ...
                                    {
                                        var math2 = regex.Match(pathPart);
                                        if (math2.Success)
                                        {
                                            newGroupingKey = pathPart;
                                        }
                                    }
                                }
                                else if ((GroupingPatternType) m_patternType[target] == GroupingPatternType.RootFolder)
                                {
                                    newGroupingKey = pathParts[0];
                                }
